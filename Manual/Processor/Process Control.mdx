...
<blockquote>
  <p>
    The data flow is used to arrange the calling logic of the processor, and supports the use of flows
    For query node data processing, JS data stream processing is suitable for more complex business logic.
  </p>
  <p>
   The `flows` processor provides us with complete data stream processing, which can provide a set of processes for data processors. Data streams can be used as processors. The namespace is
    <strong>flows.&lt;Processor name&gt;</strong>; Query nodes can reference data flows other than themselves.
  </p>
...
<Div style={{ display: "flex", justifyContent: "space-between" }}>
  ...
  ></Link>
  <Link type="next" title="Array" link="Manual/Processor/Array"></Link>
</Div>
