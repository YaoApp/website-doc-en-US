...
A data flow orchestration (`Flow`) can have multiple query nodes, each query node can call a processor (`process`), can specify the result processing script and return value, and can refer to the context information in the query node.
...
| args    | Array<Any\>    | Processor argument list. Can refer to input/output or context data                                                                                                    | yes             |
| script  | String         | Result processing JS script                                                                                                                                           | no              |
| outs    | Array<String\> | Query node result output. Use `{{$out}}` to refer to the handler to return the result. If not set, the return value is equal to the result returned by the processor. | No              |
...
The data flow (`Flow`) can be passed as a global reference parameter (`{{$in}}`) and the return value of each query node `{{$res}}`. In the `outs` array inside the query node, you can refer to the return value `{{out}}` of the processor (`process`).
...
| `{{out}}`  | Any                | Query the return value of the node's processor (`process`), support using `.` to apply Object /Array values ​​                                                                                              | `nodes[n].outs`                            | `{{out}} `, `{{out.name}}`, `{{out.0}}`                        |
...
| args[1]    | Any                | current query node processor (`process`) return value, i.e. `{{$out}}`  |
...
A data flow (`Flow`) can be used as a process to call in other data flows (`Flow`) or service interfaces (`API`). Processors are referenced as `flows.dataflow_name`
