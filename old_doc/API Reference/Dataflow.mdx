...
| outs    | Array<String\> | Query node result output. Use `{{$out}}` to refer to the process to return the result. If not set, the return value is equal to the result returned by the processor. | No              |
