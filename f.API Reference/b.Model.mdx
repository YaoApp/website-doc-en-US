...
The data model definition document consists of basic information, data tables, field definitions, index definitions, relational mappings, default data, and configuration sections. [see full example] (#5 - full example)
...
| --------- | --------------------- | ------------------------------------------------------------------------------ | --------------- |
| name      | String                | Model Chinese name [View full description](#21-Basic information)              | Yes             |
| table     | Object                | Data table definition [View full description](#22-data-table-table)            | Yes             |
| columns   | Array\<Object\>       | field definitions [View full description](#23-field-definition-columns)        | yes             |
| indexes   | Array\<Object\>       | index definition [see full description](#24-index definition-indexes)          | yes             |
| relations | \[key:String\]:Object | relation mapping [see full description](#25-relation-mapping-relations)        | no              |
| values ​​ | Array\<Object\>       | default data [see full description](#26-default-data-values)                   | no              |
| option    | Object                | Configuration options [View full description](#27-Configuration option-option) | No              |

### Basic

Basic information includes `name` , `description` , `version` and other fields, which are mainly used for presentation and retrieval in the development platform.

```json
{
	"name": "User",
	"version": "1.0.0",
	"description": "Website User Metadata Model",
	"table": {},
	"columns": [],
	"indexes": []
}
...
| Fields      | Type   | Description                                                                                                         | Required Fields |
| ----------- | ------ | ------------------------------------------------------------------------------------------------------------------- | --------------- |
| name        | String | Chinese name                                                                                                        | Yes             |
| version     | String | The version number x.x.x follows the [Semantic Versioning 2.0.0](https://semver.org/spec/v2.0.0.html) specification | yes             |
| tun         | String | Xiang Chuan Wisdom Shared Warehouse `Tun` (tun.iqka.com) address such as: `microcity/petstore/user`                 | No              |
| description | String | Data model details                                                                                                  | No              |
| author      | String | Data model author                                                                                                   | no              |
| email       | String | Data model author contact information                                                                               | No              |
| license     | String | Data model sharing protocol such as MIT                                                                             | No              |
| homepage    | String | Official website of data model provider                                                                             | No              |
...
| type        | String                 | field type, [view full description](#field type)                                                                                                          | yes             |
...
| length      | Integer                | field length, valid for `string` and other type fields [View full description](#field type)                                                               | no              |
| precision   | Integer                | Field digits (including decimal places), valid for `float`, `decimal` and other type fields [View full description](#Field type)                          | No              |
| scale       | Integer                | The number of decimal places in the field, valid for `float`, `decimal` and other type fields [View full description](#Field type)                        | No              |
| option      | Array\<String\>        | field allowable value, valid for `enum` type fields [see full description](#field type)                                                                   | no              |
...
| crypt       | String                 | Field encryption storage method (MySQL Only). Permitted values ​​`AES`, `PASSWORD` [View full description](#encryption method)                            | no              |
...
| validations | Array\<Object\>        | Field validation rules [View full description](#Validation method)                                                                                        | No              |
...
| max          | `[<Integer\|Float>]`                                                                          | max                 | `{"method":"max", "args":[0.618]}`                                     |
...
**The current relationship mapping part is a beta version, and the data structure may be adjusted based on user feedback**
...
| ------------------------- | ------------ | ------------------------------------------------------------------------------------------------------- |
| `hasOne`                  | One-to-one   | Model A is associated with Model B one-to-one [see full description](#251-hasone-one-to-one)            |
| `hasMany`                 | one-to-many  | Model A is associated with model B through one-to-many [see full description](#253-hasmany-one-to-many) |
...
| query   | Object QueryParam        | Relational query parameter default value. If the associated query parameter is not specified in the query, the query parameter defined in the model will be used instead [View full description](#3-query parameter-queryparam) | No              |
| links   | Array\<Object Relation\> | `hasOneThrough` or `hasManyThrough` multi-table relationship definition                                                                                                                                                         | no              |     | no  |
...
3. Set the association model `key` to `user_id`, that is: when the `address.user_id` engine is processed, the `user_id` field of the `address` table is automatically associated.
...
| soft_deletes | Bool | forWhen true, `deleted_at` and `__restore_data` fields are automatically created. When data is deleted, the unique field data is backed up, and the operation time is marked. The data marked for deletion is ignored when querying |

## `QueryParam`

Query conditions are described by Object `QueryParam` when the model association is defined and the process is called.
...
Query conditions can be passed in through URL Query String, [see the comparison table for details](../api/protocol/http.md#url-query-string- and -queryparam- comparison table)

## Process

The data model provides a set of atomic operation processes `process` , which can be used for service interface (`API`) and data flow (`Flow`) orchestration.

| Process           | Citation                               | Description                                                                                                                                                                                           |
| ------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ------ | -------------------------- | ---------------------------------------------------------------------------------------------- |
| find                | models. model name. Find               | query a single record [details](process/Find.md)                                                                                                                                                      |
| get                 | models. Model name. Get                | Query by condition, no pagination [Details](process/Get.md)                                                                                                                                           |
| paginate            | models. model name. Paginate           | query by condition, pagination [details](process/Paginate.md)                                                                                                                                         |     | create | models. Model name. Create | Create a single record, return the ID of the newly created record [Details](process/Create.md) |
| update              | models. model name. Update             | update a single record [details](process/Update.md)                                                                                                                                                   |
| save                | models. model name. Save               | save a single record, there is no create record, there is an update record, return record ID [details](process/Save.md)                                                                               |
| delete              | models.model name.Delete               | delete a single record (mark delete) [details](process/Delete.md)                                                                                                                                     |
| destroy             | models. model name. Destroy            | delete a single record (true delete) [details](process/Destroy.md)                                                                                                                                    |
| insert              | models.Model name.Insert               | Insert multiple records, return the number of inserted rows [Details](process/Insert.md)                                                                                                              |
| updatewhere         | models. model name. UpdateWhere        | Update records by condition, return the number of updated rows [Details](process/UpdateWhere.md)                                                                                                      |
| deletewhere         | models. model name. DeleteWhere        | delete data by condition, return the number of rows deleted (marked for deletion) [details](process/DeleteWhere.md)                                                                                   |
| destroywhere        | models. model name. DestroyWhere       | delete data by condition, return the number of rows deleted (true delete) [details](process/DestroyWhere.md)                                                                                          |
| eachsave            | models. model name.EachSave            | save multiple records, there is no created record, there is an update record, return record ID collection [details](process/EachSave.md)                                                              |
| eachsaveAfterDelete | models. model name.EachSaveAfterDelete | After deleting a set of records with a given ID, save multiple records, no created records exist, there are update records, and return record ID collection [Details](process/EachSaveAfterDelete.md) |
